...
(
    ...
    'Context JSON:\n{{context}}\n\nRead context.agent.observations and context.agent.finalAnswer. Summarize order status and callback diagnostics using only observed evidence.',
    0.00,
    'REVIEW',
...
(
    ...
    'TEXT',
    'You are an order diagnostics summarizer.',
    'Context JSON:\n{{context}}\n\nUse context.agent.finalAnswer as primary final response. Use context.agent.observations only for supporting details.',
    0.00,
    'FINAL',
...
(
    ...
    'Context JSON:\n{{context}}\n\nIf schema is incomplete, respond with exactly which required fields are missing using {{missing_fields}} and ask only for those fields. If schema is complete, read context.agent.observations and context.agent.finalAnswer when present. Mention rating, fraud flag, dti, availableCredit, and applicationId when available.',
    0.00,
    'COLLECT',
    '{"allows":["reset"],"expects":["structured_input"]}',
    1
...
(
    ...
    'TEXT',
    'You are a strict loan decision summarizer.',
    'Context JSON:\n{{context}}\n\nUse context.agent.finalAnswer as primary final answer. Use context.agent.observations only to validate details.',
    0.00,
    'FINAL',
    '{"allows":["reset"],"expects":[]}',
    1
...
('ORDER_DIAGNOSTICS', 'ANALYZE', 'TEXT', 'DERIVED',
 NULL,
 'Use context.agent.observations and context.agent.finalAnswer to diagnose submitted order and callback status.',
 NULL, 20, 1, 'Order diagnostics derived response in ANALYZE state'),
('ORDER_DIAGNOSTICS', 'COMPLETED', 'TEXT', 'DERIVED',
 NULL,
 'Use context.agent.finalAnswer as final answer. Keep concise and evidence-based from context.agent.observations.',
...
('LOAN_APPLICATION', 'ELIGIBILITY_GATE', 'TEXT', 'DERIVED',
 NULL,
 'If missing_fields is not empty, clearly list only missing required fields from {{missing_fields}} and do not claim processing/submission. Otherwise use context.agent.observations and context.agent.finalAnswer to explain decision. Reject on low rating/fraud/poor profile. Include applicationId when submitted.',
 NULL, 20, 1, 'Loan decision derived from MCP chain'),
('LOAN_APPLICATION', 'COMPLETED', 'TEXT', 'DERIVED',
 NULL,
 'Use context.agent.finalAnswer as primary answer. Validate wording with context.agent.observations and include evidence fields when present.',
...
('POST_AGENT_TOOL', 'ORDER_DIAGNOSTICS', 'ANALYZE', 'JSON_PATH',
 '$[?(@.context.agent.finalAnswer != ''null'' && @.context.agent.finalAnswer != null && @.context.agent.finalAnswer != '''')]',
 'SET_STATE', 'COMPLETED', 42, 1,
 'Move ORDER_DIAGNOSTICS to COMPLETED when context.agent.finalAnswer exists'),
...
('POST_AGENT_TOOL', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 'JSON_PATH',
 '$[?(@.context.agent.finalAnswer != null && @.context.agent.finalAnswer != '''')]',
 'SET_STATE', 'COMPLETED', 62, 1,
 'Move LOAN_APPLICATION to COMPLETED when context.agent.finalAnswer exists'),
...
-- -----------------------------------------------------------------------------
INSERT OR REPLACE INTO ce_agent_tool (tool_id, tool_code, tool_group, intent_code, state_code, enabled, description)
VALUES
...
-- -----------------------------------------------------------------------------
INSERT OR REPLACE INTO ce_agent_planner (planner_id, intent_code, state_code, system_prompt, user_prompt, enabled, created_at)
VALUES
(
    5101,
    'ORDER_DIAGNOSTICS',
    ...
    'User input:\n{{user_input}}\n\nCurrent date/time context:\n- current_date: {{current_date}}\n- current_datetime: {{current_datetime}}\n- current_year: {{current_year}}\n- current_timezone: {{current_timezone}}\n\nStandalone query:\n{{standalone_query}}\n\nRecent conversation history:\n{{conversation_history}}\n\nContext JSON:\n{{context}}\n\nAvailable Agent tools:\n{{agent_tools}}\n\nExisting Agent observations:\n{{agent_observations}}\n\nIf the user refers to prior results using words like "above", "that", "those", "same", or "previous", resolve references using standalone_query + conversation_history + agent_observations before asking clarification.\n\nReturn strict JSON:\n{\n  "action":"CALL_TOOL" | "ANSWER",\n  "tool_code":"<tool_code_or_null>",\n  "args":{},\n  "answer":"<text_or_null>"\n}',
    1,
    CURRENT_TIMESTAMP
),
(
