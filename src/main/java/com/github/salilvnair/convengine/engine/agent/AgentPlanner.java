...
public class AgentPlanner {
    ...
    public AgentPlan plan(EngineSession session, List<CeAgentTool> tools, List<AgentObservation> observations) {
        PlannerPromptSet promptSet = resolvePlannerPromptSet(session);
        List<CeAgentTool> plannerTools = filterToolsByPlannerPrompt(tools, promptSet);
        ...
        // Merge runtime-discovered MCP server tools
        if (mcpRegistry != null) {
            try {
        ...
        String toolsJson = JsonUtil.toJson(toolViews);
        ...
        try {
            ...
            AgentPlan normalized = normalizePlan(parsed);
            return rewriteSemanticPlan(normalized, observations);
        } catch (Exception e) {
            return new AgentPlan(AgentConstants.ACTION_ANSWER, null, java.util.Map.of(),
                    AgentConstants.FALLBACK_PLAN_ERROR, null);
        }
    }
    ...
    private List<CeAgentTool> filterToolsByPlannerPrompt(List<CeAgentTool> tools, PlannerPromptSet promptSet) {
    ...
    static List<CeAgentTool> filterToolsByMentionedCodes(List<CeAgentTool> tools, Set<String> mentionedToolCodes) {
        if (tools == null || tools.isEmpty()) {
            return List.of();
        }
        ...
        List<CeAgentTool> filtered = tools.stream()
    ...
    private AgentPlan normalizePlan(AgentPlan plan) {
        if (plan == null) {
            return new AgentPlan(AgentConstants.ACTION_ANSWER, null, Map.of(), AgentConstants.FALLBACK_PLAN_ERROR, null);
        }

        String rawAction = trimToNull(plan.action());
        if (rawAction != null) {
            String upper = rawAction.toUpperCase(Locale.ROOT);
            if (!AgentConstants.ACTION_CALL_TOOL.equals(upper) && !AgentConstants.ACTION_ANSWER.equals(upper)) {
                return new AgentPlan(
                        AgentConstants.ACTION_ANSWER,
                        null,
                        Map.of(),
                        AgentConstants.FALLBACK_PLAN_ERROR,
                        null
                );
            }
        }
        ...
        if (AgentConstants.ACTION_CALL_TOOL.equals(action)) {
            if (toolCode == null) {
                return new AgentPlan(AgentConstants.ACTION_ANSWER, null, Map.of(), AgentConstants.FALLBACK_UNSAFE_NEXT_STEP, null);
            }
        } else {
            args = Map.of();
        }

        return new AgentPlan(action, toolCode, args, answer, operationTag);
    }
    ...
    private String normalizeAction(String rawAction, String toolCode, String answer) {
        ...
        if (action != null) {
            ...
            if (AgentConstants.ACTION_CALL_TOOL.equals(upper)) {
                return AgentConstants.ACTION_CALL_TOOL;
            }
            if (AgentConstants.ACTION_ANSWER.equals(upper)) {
                return AgentConstants.ACTION_ANSWER;
            }
        }
        if (toolCode != null) {
            return AgentConstants.ACTION_CALL_TOOL;
    ...
    private AgentPlan rewriteSemanticPlan(AgentPlan plan, List<AgentObservation> observations) {
        if (plan == null || !AgentConstants.ACTION_CALL_TOOL.equalsIgnoreCase(trimToNull(plan.action()))) {
            return plan;
        }
        String toolCode = trimToNull(plan.tool_code());
        if (toolCode == null) {
        ...
        if ("db.semantic.query".equalsIgnoreCase(toolCode) && !args.containsKey("canonicalIntent") && !args.containsKey("canonical_intent")) {
            ...
            if (!canonicalIntent.isEmpty()) {
                ...
                return new AgentPlan(AgentConstants.ACTION_CALL_TOOL, "db.semantic.query", args, null, plan.operation_tag());
            }
        }
        return plan;
    }

    private Map<String, Object> extractCanonicalIntent(Map<String, Object> args, List<AgentObservation> observations) {
    ...
    private ObservationPayload buildObservationsPayload(List<AgentObservation> observations) {
        ...
        for (AgentObservation observation : recentObservations) {
    ...
    private Object summarizeObservationForPlanner(JsonNode parsed, String raw, int maxChars) {
        ...
        for (String key : AgentConstants.OBSERVATION_SUMMARY_FIELDS) {
