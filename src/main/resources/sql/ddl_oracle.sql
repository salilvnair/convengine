DROP TABLE ce_agent_db_tool;
DROP TABLE ce_agent_planner;
DROP TABLE ce_semantic_entity;
DROP TABLE ce_semantic_relationship;
DROP TABLE ce_semantic_join_hint;
DROP TABLE ce_semantic_value_pattern;
DROP TABLE ce_user_query_knowledge;
DROP TABLE ce_agent_user_feedback;
DROP TABLE ce_agent_query_knowledge;
DROP TABLE ce_conversation_history;
DROP TABLE ce_audit;
DROP TABLE ce_pending_action;
DROP TABLE ce_verbose;
DROP TABLE ce_rule;
DROP TABLE ce_response;
DROP TABLE ce_prompt_template;
DROP TABLE ce_policy;
DROP TABLE ce_output_schema;
DROP TABLE ce_agent_tool;
...
CREATE TABLE ce_agent_tool (
  ...
  CONSTRAINT ce_agent_tool_intent_code_not_blank CHECK (TRIM(intent_code) IS NOT NULL),
  CONSTRAINT ce_agent_tool_state_code_not_blank CHECK (TRIM(state_code) IS NOT NULL)
);
...
CREATE TABLE ce_output_schema (
  ...
  intent_code VARCHAR2(255) NOT NULL,
  state_code VARCHAR2(255) NOT NULL,
  schema_type VARCHAR2(50) DEFAULT 'SCHEMA_JSON' NOT NULL,
...
CREATE TABLE ce_verbose (
  verbose_id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  intent_code VARCHAR2(255) NOT NULL,
  state_code VARCHAR2(255) NOT NULL,
  step_match VARCHAR2(50) DEFAULT 'EXACT' NOT NULL,
  step_value VARCHAR2(255) NOT NULL,
  determinant VARCHAR2(255) NOT NULL,
  rule_id NUMBER(19),
  tool_code VARCHAR2(255),
  message CLOB,
  error_message CLOB,
  priority NUMBER(10) DEFAULT 100 NOT NULL,
  enabled NUMBER(1) DEFAULT 1 NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
  CONSTRAINT ce_verbose_intent_not_blank CHECK (TRIM(intent_code) IS NOT NULL),
  CONSTRAINT ce_verbose_state_not_blank CHECK (TRIM(state_code) IS NOT NULL),
  CONSTRAINT ce_verbose_step_match_not_blank CHECK (TRIM(step_match) IS NOT NULL),
  CONSTRAINT ce_verbose_step_value_not_blank CHECK (TRIM(step_value) IS NOT NULL),
  CONSTRAINT ce_verbose_determinant_not_blank CHECK (TRIM(determinant) IS NOT NULL)
);
...
CREATE TABLE ce_agent_planner (
  ...
  enabled NUMBER(1) DEFAULT 1 NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
  CONSTRAINT ce_agent_planner_intent_not_blank CHECK (TRIM(intent_code) IS NOT NULL),
  CONSTRAINT ce_agent_planner_state_not_blank CHECK (TRIM(state_code) IS NOT NULL)
);
CREATE INDEX idx_ce_agent_planner_scope ON ce_agent_planner (enabled, intent_code, state_code, planner_id);

CREATE TABLE ce_agent_query_knowledge (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE ce_agent_user_feedback (
  ...
  agent_tool_code VARCHAR2(255),
  captured_query_knowledge_count NUMBER(10) DEFAULT 0 NOT NULL,
  applied_query_knowledge_json CLOB,
  metadata_json CLOB,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TABLE ce_user_query_knowledge (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  conversation_id RAW(16),
  feedback_id NUMBER,
  feedback_type VARCHAR2(32),
  tool_code VARCHAR2(255),
  intent_code VARCHAR2(255),
  state_code VARCHAR2(255),
  query_text VARCHAR2(1000) NOT NULL,
...
CREATE TABLE ce_semantic_join_hint (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  base_table VARCHAR2(255) NOT NULL,
  join_table VARCHAR2(255) NOT NULL,
  priority NUMBER(10) DEFAULT 100 NOT NULL,
  enabled NUMBER(1) DEFAULT 1 NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
...
CREATE TABLE ce_semantic_entity (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  entity_name VARCHAR2(255) NOT NULL,
  description CLOB,
  primary_table VARCHAR2(255),
  related_tables VARCHAR2(2000),
...
CREATE TABLE ce_semantic_value_pattern (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  from_field VARCHAR2(255) NOT NULL,
  to_field VARCHAR2(255) NOT NULL,
  value_starts_with VARCHAR2(2000) NOT NULL,
  priority NUMBER(10) DEFAULT 100 NOT NULL,
  enabled NUMBER(1) DEFAULT 1 NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
