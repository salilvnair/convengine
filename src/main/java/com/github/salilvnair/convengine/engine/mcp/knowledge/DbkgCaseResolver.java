...
public class DbkgCaseResolver {
    ...
    public Map<String, Object> resolveCase(Map<String, Object> args, EngineSession session) {
        ...
        List<Map<String, Object>> rankedCases = support.rankSignals(cases, signals, DbkgConstants.COL_CASE_CODE, question, tokens);
    ...
    public List<Map<String, Object>> rankPlaybooks(Map<String, Object> selectedCase, String question, List<String> tokens) {
        if (selectedCase == null || selectedCase.get(DbkgConstants.KEY_CASE_CODE) == null) {
            return List.of();
        }
        String caseCode = String.valueOf(selectedCase.get(DbkgConstants.KEY_CASE_CODE));
        List<Map<String, Object>> playbooks = support.readEnabledRows(support.cfg().getPlaybookTable()).stream()
                .filter(row -> caseCode.equalsIgnoreCase(support.asText(row.get(DbkgConstants.COL_CASE_CODE))))
                .toList();
        List<Map<String, Object>> signals = support.readEnabledRows(support.cfg().getPlaybookSignalTable());
        return support.rankSignals(playbooks, signals, DbkgConstants.COL_PLAYBOOK_CODE, question, tokens);
    }
}
