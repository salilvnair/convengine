-- ConvEngine 2.0.7: Advanced MCP seed (Postgres)
-- Purpose:
...
VALUES
  (9001, 'db.knowledge.graph', 'DB', 'ANY', 'ANY', true,
   'Semantic query/schema knowledge retrieval for similar issue diagnosis', NOW()),
  (9002, 'order.status.api', 'HTTP_API', 'ANY', 'ANY', true,
   'Order status lookup via external API with retry/circuit/auth/mapping', NOW())
...
-- ---------------------------------------------------------------------------
INSERT INTO ce_mcp_query_knowledge (id, query_text, description, prepared_sql, tags, api_hints)
VALUES
  (
    10001,
  ...
  (
    ...
    'SELECT o.order_id, o.created_at, o.callback_received_at,\n       EXTRACT(EPOCH FROM (o.callback_received_at - o.created_at)) AS callback_latency_seconds\nFROM orders o\nWHERE o.created_at >= NOW() - INTERVAL ''7 days''\nORDER BY callback_latency_seconds DESC NULLS LAST\nLIMIT :limit;',
...
SET
  ...
  description = EXCLUDED.description,
  prepared_sql = EXCLUDED.prepared_sql,
  tags = EXCLUDED.tags,
  api_hints = EXCLUDED.api_hints;
