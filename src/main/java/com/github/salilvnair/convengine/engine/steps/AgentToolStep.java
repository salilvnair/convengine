...
import com.github.salilvnair.convengine.engine.agent.AgentPlanner;
...
import com.github.salilvnair.convengine.engine.agent.model.AgentObservation;
import com.github.salilvnair.convengine.engine.agent.model.AgentPlan;
...
public class AgentToolStep implements EngineStep {
    ...
    public StepResult execute(EngineSession session) {
        ...
        for (int i = 0; i < maxLoops; i++) {
            ...
            if (AgentConstants.ACTION_ANSWER.equalsIgnoreCase(plan.action())) {
            ...
            if (!AgentConstants.ACTION_CALL_TOOL.equalsIgnoreCase(plan.action())) {
            ...
            if (executedToolSignatures.contains(toolSignature)) {
                ...
                if (duplicateLoopAnswer == null || duplicateLoopAnswer.isBlank()) {
                    duplicateLoopAnswer = AgentConstants.FALLBACK_UNSAFE_NEXT_STEP;
                }
            ...
            try {
                applyToolDelay(i + 1);
                AgentToolExecutor executor = resolveExecutor(toolGroup);
                ...
                if (TOOL_DB_SEMANTIC_INTERPRET.equalsIgnoreCase(toolCode)) {
                    rulesStep.applyRules(session, "AgentToolStep PostSemanticInterpret",
    ...
    private DialogueAct parseDialogueAct(String raw) {
        if (raw == null || raw.isBlank() || AgentConstants.NULL_LITERAL.equalsIgnoreCase(raw)) {
            return null;
        }
        try {
    ...
    private String resolveToolErrorMessage(EngineSession session, String toolCode, Map<String, Object> metadata) {
        return AgentConstants.FALLBACK_TOOL_ERROR;
    }
    ...
    private List<AgentObservation> readObservationsFromContext(EngineSession session) {
        try {
            ...
            List<AgentObservation> list = new ArrayList<>();
            for (JsonNode n : obs) {
                String tool = n.path(AgentConstants.CONTEXT_OBSERVATION_TOOL_CODE).asText(null);
                String json = n.path(AgentConstants.CONTEXT_OBSERVATION_JSON).asText(null);
                if (tool != null && json != null)
                    list.add(new AgentObservation(tool, json));
            }
            return list;
        } catch (Exception e) {
            return new ArrayList<>();
    ...
    private void writeObservationsToContext(EngineSession session, List<AgentObservation> observations) {
        try {
            ...
            for (AgentObservation o : observations) {
                ...
                n.put(AgentConstants.CONTEXT_OBSERVATION_TOOL_CODE, o.toolCode());
                n.put(AgentConstants.CONTEXT_OBSERVATION_JSON, o.json());
                arr.add(n);
            }
            mcp.set(AgentConstants.CONTEXT_KEY_OBSERVATIONS, arr);
    ...
    private boolean isAllowedByNextToolGuardrail(String nextToolCode, List<AgentObservation> observations) {
        return nextToolGuardrailBlockReason(nextToolCode, observations) == null;
    }

    private String nextToolGuardrailBlockReason(String nextToolCode, List<AgentObservation> observations) {
    ...
    private boolean isAllowedBySemanticPipelineGuard(String nextToolCode, List<AgentObservation> observations) {
        if (nextToolCode == null || nextToolCode.isBlank() || observations == null || observations.isEmpty()) {
            return true;
        }
        AgentObservation last = observations.get(observations.size() - 1);
    ...
    private boolean isAllowedByConfiguredNextToolGuardrail(String nextToolCode, List<AgentObservation> observations) {
        ConvEngineAgentConfig.Guardrail guardrail =
                mcpConfig == null || mcpConfig.getGuardrail() == null
                        ? new ConvEngineAgentConfig.Guardrail()
    ...
    private void writeLifecycleToContext(
        ...
        try {
            ...
            lifecycle.put(AgentConstants.CONTEXT_KEY_BLOCKED, blocked);
            lifecycle.put(AgentConstants.CONTEXT_KEY_ERROR, error);
            lifecycle.put(AgentConstants.CONTEXT_KEY_TOOL_EXECUTED, AgentConstants.STATUS_TOOL_RESULT.equalsIgnoreCase(status));

            if (lastAction != null) {
                lifecycle.put(AgentConstants.CONTEXT_KEY_LAST_ACTION, lastAction);
            } else {
                lifecycle.remove(AgentConstants.CONTEXT_KEY_LAST_ACTION);
            }
            if (lastToolCode != null) {
                lifecycle.put(AgentConstants.CONTEXT_KEY_LAST_TOOL_CODE, lastToolCode);
            } else {
                lifecycle.remove(AgentConstants.CONTEXT_KEY_LAST_TOOL_CODE);
            }
            if (lastToolGroup != null) {
                lifecycle.put(AgentConstants.CONTEXT_KEY_LAST_TOOL_GROUP, lastToolGroup);
            } else {
                lifecycle.remove(AgentConstants.CONTEXT_KEY_LAST_TOOL_GROUP);
            }
            if (lastToolArgs != null && !lastToolArgs.isEmpty()) {
                lifecycle.set(AgentConstants.CONTEXT_KEY_LAST_TOOL_ARGS, mapper.valueToTree(lastToolArgs));
            } else {
                lifecycle.remove(AgentConstants.CONTEXT_KEY_LAST_TOOL_ARGS);
            }
            if (errorMessage != null && !errorMessage.isBlank()) {
                lifecycle.put(AgentConstants.CONTEXT_KEY_ERROR_MESSAGE, errorMessage);
            } else {
                lifecycle.remove(AgentConstants.CONTEXT_KEY_ERROR_MESSAGE);
            }
    ...
    private String latestObservationSummary(List<AgentObservation> observations) {
    ...
    private void writeSemanticLifecycle(
        ...
        if (lastToolCode != null && !lastToolCode.isBlank()) {
            ...
            if (AgentConstants.STATUS_TOOL_RESULT.equalsIgnoreCase(status)) {
    ...
    private JsonNode latestObservationNode(List<AgentObservation> observations, String toolCode) {
