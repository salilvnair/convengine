...
1. `src/main/java/com/github/salilvnair/convengine/config/ConvEngineTransportConfig.java` (new)
   ...
   - `Broker` config:
   - `mode` (`SIMPLE` default, optional `RELAY`)
   - relay host/port, destination prefixes, credentials, heartbeat settings
...
### 10) Production-Grade Audit Dispatch & Controls
1. `src/main/java/com/github/salilvnair/convengine/config/ConvEngineAuditConfig.java` (new)
   - Binds `convengine.audit.*`.
   - Adds global audit controls:
   - `enabled`
   - `level` (`ALL`, `STANDARD`, `ERROR_ONLY`, `NONE`)
   - `includeStages` / `excludeStages` (wildcard support)
   - `dispatch` (async thread/queue/backpressure policy)
   - `rateLimit` (window-based event throttling)

2. `src/main/java/com/github/salilvnair/convengine/audit/dispatch/AuditStageControl.java` (new)
   - Decides if a stage should be audited.
   - Applies level filtering, include/exclude wildcard matching, and rate-limit checks.
   - Supports per-conversation/per-stage buckets with bounded tracked bucket count.

3. `src/main/java/com/github/salilvnair/convengine/audit/dispatch/AuditEventDispatcher.java` (new)
   - Centralized listener dispatch abstraction.
   - Default: synchronous dispatch (existing behavior).
   - Optional async mode with bounded queue and configurable rejection policy:
   - `CALLER_RUNS`, `DROP_NEWEST`, `DROP_OLDEST`, `ABORT`.

4. `src/main/java/com/github/salilvnair/convengine/audit/DbAuditService.java` (updated)
   - Calls `AuditStageControl.shouldAudit(...)` before persisting.
   - Dispatches events via `AuditEventDispatcher` instead of direct listener loop.
   - Keeps default runtime behavior unchanged unless audit config/annotation enables async controls.

### 11) Annotation-Based Feature Toggles
1. `src/main/java/com/github/salilvnair/convengine/annotation/EnableConvEngine.java` (updated)
   - Added attribute: `stream` (default `true`).
   - `stream=true`:
   - framework enforces startup validation that at least one transport is enabled (`sse` or `stomp`).
   - `stream=false`:
   - SSE/STOMP beans are not activated even if transport properties are enabled; REST-only flow remains active.

2. `src/main/java/com/github/salilvnair/convengine/annotation/EnableConvEngineAsyncAuditDispatch.java` (new)
   - Enables async audit dispatch marker in consumer app.

3. `src/main/java/com/github/salilvnair/convengine/annotation/EnableConvEngineStompBrokerRelay.java` (new)
   - Enables STOMP broker relay marker in consumer app.

4. Stream support classes:
   - `src/main/java/com/github/salilvnair/convengine/config/stream/EnableConvEngineImportRegistrar.java`
   - `src/main/java/com/github/salilvnair/convengine/config/stream/ConvEngineStreamSettings.java`
   - `src/main/java/com/github/salilvnair/convengine/config/stream/ConvEngineStreamEnabledCondition.java`
   - `src/main/java/com/github/salilvnair/convengine/config/stream/ConvEngineStreamStartupValidator.java`

5. Marker classes:
   - `src/main/java/com/github/salilvnair/convengine/config/feature/ConvEngineAsyncAuditDispatchMarker.java`
   - `src/main/java/com/github/salilvnair/convengine/config/feature/ConvEngineStompBrokerRelayMarker.java`

### 12) Experimental SQL Generation
...
10. `convengine.transport.stomp.broker.mode`
11. `convengine.transport.stomp.broker.relay-destination-prefixes`
12. `convengine.transport.stomp.broker.relay-host`
13. `convengine.transport.stomp.broker.relay-port`
14. `convengine.transport.stomp.broker.client-login`
15. `convengine.transport.stomp.broker.client-passcode`
16. `convengine.transport.stomp.broker.system-login`
17. `convengine.transport.stomp.broker.system-passcode`
18. `convengine.transport.stomp.broker.virtual-host`
19. `convengine.transport.stomp.broker.system-heartbeat-send-interval-ms`
20. `convengine.transport.stomp.broker.system-heartbeat-receive-interval-ms`
21. `convengine.audit.enabled`
22. `convengine.audit.level`
23. `convengine.audit.include-stages`
24. `convengine.audit.exclude-stages`
25. `convengine.audit.dispatch.async-enabled`
26. `convengine.audit.dispatch.worker-threads`
27. `convengine.audit.dispatch.queue-capacity`
28. `convengine.audit.dispatch.rejection-policy`
29. `convengine.audit.dispatch.keep-alive-seconds`
30. `convengine.audit.rate-limit.enabled`
31. `convengine.audit.rate-limit.max-events`
32. `convengine.audit.rate-limit.window-ms`
33. `convengine.audit.rate-limit.per-conversation`
34. `convengine.audit.rate-limit.per-stage`
35. `convengine.audit.rate-limit.max-tracked-buckets`
36. `convengine.audit.persistence.mode`
37. `convengine.audit.persistence.jdbc-batch-size`
38. `convengine.audit.persistence.max-buffered-events`
39. `convengine.audit.persistence.flush-stages`
40. `convengine.audit.persistence.final-step-names`
41. `convengine.audit.persistence.flush-on-stop-outcome`
42. `convengine.experimental.enabled`
43. `ce_config.RESET_INTENT_CODES` (comma-separated, default `RESET_SESSION`)
