...
-- Purpose:
-- 1) Seed DB MCP tools (db.semantic.catalog + postgres.query)
-- 2) Seed a sample advanced HTTP_API tool (order.status.api)
-- 3) Seed query/schema knowledge catalogs used by DbSemanticCatalogToolHandler
...
VALUES
  (9001, 'db.semantic.catalog', 'DB', 'ANY', 'ANY', true,
   'Semantic query/schema knowledge retrieval for similar issue diagnosis', NOW()),
  (9003, 'postgres.query', 'DB', 'ANY', 'ANY', true,
   'Execute raw read-only Postgres SQL generated by the LLM.', NOW()),
...
-- No ce_mcp_db_tool row is required for db.semantic.catalog when handled by
