...
import com.github.salilvnair.convengine.engine.agent.AgentDbExecutor;
...
public class DbToolExecutor implements AgentToolExecutor {

    private final AgentDbExecutor dbExecutor;
    ...
    public String execute(CeAgentTool tool, Map<String, Object> args, EngineSession session) {
        ...
        if (!AgentConstants.TOOL_GROUP_DB.equalsIgnoreCase(normalizedGroup)) {
            throw new IllegalStateException(
                    "DbToolExecutor can execute only DB tools. toolCode=" + toolCode + ", toolGroup=" + toolGroup);
        }
        if (isSemanticQueryToolCode(toolCode)) {
            throw new IllegalStateException(buildSemanticQueryHandlerMissingMessage(toolCode));
        }
        CeAgentDbTool dbTool = registry.requireDbTool(toolCode);
    ...
    private String normalizeResult(Object value) {
        if (value == null) {
            return AgentConstants.EMPTY_JSON_OBJECT;
        }
        try {
